...
The loss for \\( x_n \\) is: $$ L_n = x_n(p_n -1)-log(1+e^{-x_n}).$$ However, the range for \\( e^{-x_n} \in (1,\infty]\\) when $$ x_n<0 $$. To avoid this, the aurthor uses \\(-log(1+e^{-x_n})=log\frac{1}{1+e^{-x_n}}=log \frac{e^{x_n}}{e^{x_n}+1}=x_n-log(1+e^{x_n}) \\) to change the range into (0,1).
